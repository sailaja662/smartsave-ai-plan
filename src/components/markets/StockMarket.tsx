...
const STOCKS = [
  {
    ...
    data: [2500, 2620, 2580, 2720, 2800, 2750, 2923], currency: "₹", exchange: "NSE/BSE",
  },
  {
    ...
    data: [3500, 3580, 3620, 3700, 3750, 3800, 3842], currency: "₹", exchange: "NSE/BSE",
  },
  {
    ...
    data: [1700, 1680, 1640, 1590, 1610, 1638, 1621], currency: "₹", exchange: "NSE/BSE",
  },
  {
    ...
    data: [1450, 1520, 1580, 1620, 1650, 1672, 1689], currency: "₹", exchange: "NSE/BSE",
  },
  {
    symbol: "AAPL", name: "Apple Inc.", sector: "Tech", price: 214, change: 0.95, marketCap: "$3.3T",
    pe: 34.2, high52w: 260, low52w: 164, growth: "High", risk: "Low",
    about: "World's most valuable company — iPhone, Mac, Services & Wearables ecosystem.",
    data: [178, 185, 191, 200, 208, 211, 214], currency: "$", exchange: "NASDAQ",
  },
  {
    symbol: "TSLA", name: "Tesla, Inc.", sector: "EV/Auto", price: 248, change: 2.84, marketCap: "$790B",
    pe: 62.1, high52w: 488, low52w: 138, growth: "Very High", risk: "High",
    about: "Global leader in electric vehicles, energy storage & autonomous driving.",
    data: [148, 162, 175, 198, 222, 241, 248], currency: "$", exchange: "NASDAQ",
  },
  {
    ...
    data: [5900, 6100, 6300, 6500, 6700, 6780, 6842], currency: "₹", exchange: "NSE/BSE",
  },
  {
    ...
    data: [10200, 10500, 10800, 11000, 11200, 11350, 11420], currency: "₹", exchange: "NSE/BSE",
  },
];

const INVEST_PLATFORMS = [
  { name: "Groww", url: "https://groww.in", color: "#00d09c", icon: "🌱" },
  { name: "Zerodha", url: "https://zerodha.com", color: "#387ed1", icon: "⚡" },
  { name: "Upstox", url: "https://upstox.com", color: "#f97316", icon: "📈" },
];

const SECTOR_COLORS: Record<string, string> = {
  IT: "#22d3ee", Energy: "#f97316", Banking: "#4ade80", NBFC: "#a78bfa",
  "Real Estate": "#facc15", Auto: "#fb7185", Pharma: "#34d399", Tech: "#818cf8", "EV/Auto": "#34d399",
};

const RISK_COLORS: Record<string, string> = {
  Low: "text-green-success", Medium: "text-yellow-warn", High: "text-red-danger", "Very High": "text-red-danger",
};
...
export function StockMarket() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        <p className="text-xs font-semibold text-muted-foreground mb-3">🚀 Invest through official SEBI-registered platforms:</p>
        ...
        {filtered.map((s, i) => (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div>
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{s.exchange}</span>
                </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-lg font-black text-foreground">{s.currency}{s.price.toLocaleString('en-IN')}</p>
                <p className="text-[10px] text-muted-foreground">{s.marketCap} mcap</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-muted-foreground">Risk</p>
                <p className={`text-xs font-bold ${RISK_COLORS[s.risk]}`}>{s.risk}</p>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      {selected && (
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            <button onClick={() => setSelected(null)} className="text-muted-foreground hover:text-foreground text-lg font-bold ml-4">✕</button>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            {[
              { label: "Current Price", val: `${selected.currency}${selected.price.toLocaleString('en-IN')}` },
              { label: "P/E Ratio", val: selected.pe.toString() },
              { label: "52W High", val: `${selected.currency}${selected.high52w.toLocaleString('en-IN')}` },
              { label: "52W Low", val: `${selected.currency}${selected.low52w.toLocaleString('en-IN')}` },
              ...
              <div key={label} className="rounded-xl p-3 bg-muted">
                <p className="text-[10px] text-muted-foreground">{label}</p>
                <p className="text-sm font-bold text-foreground">{val}</p>
              </div>
            ))}
          </div>
          <p className="text-xs text-muted-foreground mb-3 font-medium">📲 Start investing on these official platforms:</p>
          <div className="flex flex-wrap gap-2">
            {INVEST_PLATFORMS.map(p => (
              <a
                ...
                style={{ background: `${p.color}18`, color: p.color, border: `1.5px solid ${p.color}40` }}
              >
                {p.icon} Invest on {p.name} <ExternalLink size={11} />
              </a>
            ))}
          </div>
        </motion.div>
